---
title: "POST /api/v1/admin/imagenes/subir — Upload a Product Image"
sidebarTitle: "Upload Image"
description: "POST /api/v1/admin/imagenes/subir — Accepts a multipart/form-data file upload (max 5 MB, JPEG/PNG/WebP/GIF) and returns the public URL path for use in product creation."
---
...
- **Accepted MIME types:** `image/jpeg`, `image/png`, `image/webp`, `image/gif`. Any other type is rejected with `TIPO_INVALIDO`.
- **Storage location:** Files are saved to the server's local filesystem under `/recursos/imagenes/productos/`.
